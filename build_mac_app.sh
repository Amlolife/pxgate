...
APP_NAME="pxgate"
BUNDLE_ID="com.amlolife.pxgate"

log_info "pxgate v${VERSION} 빌드 시작..."
...
# 필수 파일 존재 확인
required_files=("pxgate.py" "app_icon.icns" "resources" "$VERSION_FILE")
...
rm -f pxgate.spec 2>/dev/null || true
...
# PyInstaller 실행
log_info "PyInstaller를 사용하여 pxgate 앱 빌드 중..."

# spec 파일 생성 완전 방지 - 임시 디렉토리에서 빌드
log_info "임시 디렉토리에서 spec 파일 없이 빌드 시작..."

# 임시 빌드 디렉토리 생성
TEMP_BUILD_DIR="/tmp/pxgate_build_$$"
mkdir -p "${TEMP_BUILD_DIR}"
cp pxgate.py "${TEMP_BUILD_DIR}/"
...
if [[ $? -ne 0 ]]; then
    ...
    # 임시 빌드 디렉토리 정리
    sudo rm -rf /tmp/pxgate_* 2>/dev/null || true
    ...
    # 재시도 (임시 디렉토리에서 spec 파일 생성 방지)
     ...
     # 재시도용 임시 빌드 디렉토리 생성
     RETRY_TEMP_BUILD_DIR="/tmp/pxgate_retry_build_$$"
     mkdir -p "${RETRY_TEMP_BUILD_DIR}"
     cp pxgate.py "${RETRY_TEMP_BUILD_DIR}/"
...
# 배포 디렉토리 생성
DIST_DIR="pxgate_v${VERSION}_macOS"
mkdir -p "$DIST_DIR"
...
pxgate v${VERSION} 설치 안내
================================

1. pxgate.app을 Applications 폴더로 드래그하여 설치하세요.

2. 처음 실행 시 보안 경고가 나타나면:
   - 시스템 설정 > 개인정보 보호 및 보안 > 보안
   - "확인되지 않은 개발자의 앱 허용" 섹션에서 pxgate 허용

3. 그래도 실행되지 않으면 터미널에서 다음 명령어를 실행하세요:
   xattr -rd com.apple.quarantine /Applications/pxgate.app
...
echo "  2. 사용자는 압축 해제 후 pxgate.app을 Applications 폴더로 이동"
