...
from PySide6.QtWidgets import (QApplication, QButtonGroup, QCheckBox, QComboBox,
                              ...
                              QScrollArea, QSizePolicy, QSplitter, QTextBrowser, QTextEdit,
...
def setup_logger():
    ...
    try:
        ...
        log_files = [f for f in os.listdir(log_dir) if f.startswith("pxgate_") and f.endswith(".log")]
...
class TypographySystem:
    """Professional typography system for text hierarchy"""
    # Font sizes
    SIZE_SMALL = 9
    SIZE_BODY = 10
    SIZE_MEDIUM = 11
    ...
    def get_section_title_style(cls):
        """Style for section titles (Zoom, Grid, etc.)"""
        return f"""
            QLabel {{
                color: #FFFFFF;
                font-size: {cls.SIZE_LARGE}pt;
    ...
    @classmethod
    def get_body_text_style(cls):
        """Style for regular body text"""
        return f"""
            QLabel {{
                color: #E8E8E8;
                font-size: {cls.SIZE_BODY}pt;
                font-weight: {cls.WEIGHT_NORMAL};
            }}
        """
    
    @classmethod
    def get_secondary_text_style(cls):
        """Style for secondary/metadata text"""
        return f"""
            QLabel {{
                color: #AAAAAA;
                font-size: {cls.SIZE_SMALL}pt;
                font-weight: {cls.WEIGHT_NORMAL};
            }}
        """

class SemanticColorSystem:
    """Semantic color system for different action types"""
    # Action colors
    SUCCESS = "#08E25F"      # Green - positive actions
    DANGER = "#FF4444"       # Red - destructive actions
    WARNING = "#FF9800"      # Orange - warnings
    INFO = "#6B9BD1"         # Blue - informational (current accent)
    
    # Hover states
    SUCCESS_HOVER = "#0AFF6E"
    DANGER_HOVER = "#FF5555"
    WARNING_HOVER = "#FFA726"
    INFO_HOVER = "#7DAEE0"
    
    # Pressed states
    SUCCESS_PRESSED = "#07C350"
    DANGER_PRESSED = "#E03333"
    WARNING_PRESSED = "#E08700"
    INFO_PRESSED = "#5A88BD"
    ...
    def get_success_button_style(cls):
        ...
        return f"""
            ...
            QPushButton:hover {{
                ...
                border-color: {cls.SUCCESS_HOVER};
            }}
            QPushButton:pressed {{
                background-color: {cls.SUCCESS_PRESSED};
                border-color: {cls.SUCCESS_PRESSED};
            }}
        """
    ...
    def get_danger_button_style(cls):
        ...
        return f"""
            ...
            QPushButton:hover {{
                ...
                color: #FFFFFF;
                border-color: {cls.DANGER_HOVER};
            }}
            QPushButton:pressed {{
                background-color: {cls.DANGER_PRESSED};
                border-color: {cls.DANGER_PRESSED};
            }}
        """

class SpacingSystem:
    """Professional spacing system based on 4px/8px grid"""
    ...
    @classmethod
    def get(cls, level="default"):
        """Get spacing value by level name"""
        mapping = {
            "tight": cls.TIGHT,
            "default": cls.DEFAULT,
            "comfortable": cls.COMFORTABLE,
            "loose": cls.LOOSE,
            "extra_loose": cls.EXTRA_LOOSE
        }
        return mapping.get(level.lower(), cls.DEFAULT)

class ShadowSystem:
    """Professional shadow system for depth and elevation"""
    # Shadow levels with rgba values for dark theme
    ...
    GLOW_SUBTLE = "0 0 8px rgba(107,155,209,0.2)"   # Subtle accent glow
    GLOW_MEDIUM = "0 0 16px rgba(107,155,209,0.3)"  # Medium accent glow
    GLOW_STRONG = "0 0 24px rgba(107,155,209,0.4)"  # Strong accent glow
    
    @classmethod
    def get_level(cls, level=2):
        """Get shadow by level (1-4)"""
        levels = {
            1: cls.LEVEL_1,
            2: cls.LEVEL_2,
            3: cls.LEVEL_3,
        ...
        return levels.get(level, cls.LEVEL_2)
    
    @classmethod
    def get_glow(cls, intensity="subtle"):
        """Get glow effect by intensity"""
        glows = {
            "subtle": cls.GLOW_SUBTLE,
            "medium": cls.GLOW_MEDIUM,
            "strong": cls.GLOW_STRONG
        }
        return glows.get(intensity.lower(), cls.GLOW_SUBTLE)

class BorderRadiusSystem:
    """Professional border radius system for consistent rounding"""
    ...
    @classmethod
    def get(cls, level="comfortable"):
        """Get border radius by level name"""
        mapping = {
            "subtle": cls.SUBTLE,
            "default": cls.DEFAULT,
            "comfortable": cls.COMFORTABLE,
            "prominent": cls.PROMINENT,
            "extra_prominent": cls.EXTRA_PROMINENT
        }
        return mapping.get(level.lower(), cls.COMFORTABLE)

class UIScaleManager:
    """해상도와 화면 비율에 따라 UI 크기를 동적으로 관리하는 클래스 (Enhanced with Adaptive Density)"""

    # Adaptive density levels
    DENSITY_ULTRA_COMPACT = "ultra_compact"  # For very small screens or many images
    DENSITY_COMPACT = "compact"              # Current compact mode
    DENSITY_NORMAL = "normal"                # Current normal mode
    DENSITY_COMFORTABLE = "comfortable"      # Spacious for large screens
    DENSITY_LUXURIOUS = "luxurious"          # Maximum spacing for huge screens
    
    _current_density_level = DENSITY_NORMAL
    _adaptive_mode_enabled = True  # Enable adaptive density by default
    
    # min/max 너비 개념을 다시 사용합니다.
    NORMAL_SETTINGS = {
        "control_panel_margins": (12, 14, 12, 14),     # 컨트롤 패널 내부 여백 (좌, 상, 우, 하) - Increased for comfort
        "control_layout_spacing": 12,               # 컨트롤 레이아웃 위젯 간 기본 간격 - Increased from 8
        "button_padding": 9,                       # 버튼 내부 패딩 - Increased from 7
        "delete_button_width": 45,                 # 분류폴더 번호 및 삭제(X) 버튼 너비
        "JPG_RAW_spacing": 12,                      # JPG와 RAW 폴더 섹션 사이 간격 - Increased from 8
        "section_spacing": 24,                     # 구분선(HorizontalLine) 주변 간격 - Increased from 20
        "group_box_spacing": 18,                   # 라디오 버튼 등 그룹 내 간격 - Increased from 15
        "title_spacing": 12,                       # Zoom, Grid 등 섹션 제목 아래 간격 - Increased from 10
        ...
        "folder_container_spacing": 8,             # 분류폴더 번호버튼 - 레이블 - X버튼 간격 - Increased from 6
        "folder_label_padding": 24,                # 폴더 경로 레이블 높이 계산용 패딩 - Increased from 20
        "sort_folder_label_padding": 28,           # 분류폴더 레이블 패딩 - Increased from 25
        "category_folder_vertical_spacing": 14,    # 분류 폴더 UI 사이 간격 - Increased from 10
    ...
    @classmethod
    def set_adaptive_density(cls, enabled=True):
        """Enable or disable adaptive density mode"""
        cls._adaptive_mode_enabled = enabled
        logging.info(f"Adaptive density mode: {'enabled' if enabled else 'disabled'}")
    
    @classmethod
    def adjust_density_for_image_count(cls, image_count):
        """Adaptively adjust UI density based on number of loaded images"""
        if not cls._adaptive_mode_enabled:
            return
        
        # Determine optimal density based on image count
        if image_count > 1000:
            target_density = cls.DENSITY_ULTRA_COMPACT
        elif image_count > 500:
            target_density = cls.DENSITY_COMPACT
        elif image_count > 100:
            target_density = cls.DENSITY_NORMAL
        else:
            # For smaller collections, use screen size to determine density
            screen = QGuiApplication.primaryScreen()
            if screen:
                geo = screen.geometry()
                if geo.width() >= 3840:  # 4K or higher
                    target_density = cls.DENSITY_LUXURIOUS
                elif geo.width() >= 2560:  # QHD
                    target_density = cls.DENSITY_COMFORTABLE
                else:
                    target_density = cls.DENSITY_NORMAL
            else:
                target_density = cls.DENSITY_NORMAL
        
        if target_density != cls._current_density_level:
            cls._current_density_level = target_density
            cls._apply_density_adjustments()
            logging.info(f"Adaptive density adjusted to: {target_density} (images: {image_count})")
    
    @classmethod
    def _apply_density_adjustments(cls):
        """Apply density-specific adjustments to current settings"""
        density_multipliers = {
            cls.DENSITY_ULTRA_COMPACT: 0.7,
            cls.DENSITY_COMPACT: 0.85,
            cls.DENSITY_NORMAL: 1.0,
            cls.DENSITY_COMFORTABLE: 1.15,
            cls.DENSITY_LUXURIOUS: 1.3
        }
        
        multiplier = density_multipliers.get(cls._current_density_level, 1.0)
        
        # Adjust spacing values
        spacing_keys = ["control_layout_spacing", "section_spacing", "group_box_spacing",
                       "title_spacing", "category_folder_vertical_spacing"]
        
        for key in spacing_keys:
            if key in cls._current_settings:
                base_value = cls.NORMAL_SETTINGS.get(key, cls._current_settings[key])
                cls._current_settings[key] = int(base_value * multiplier)
    
    @classmethod
    def get_current_density(cls):
        """Get current density level"""
        return cls._current_density_level

class ThemeManager:
    """Enhanced theme manager with professional dark mode variants"""
    
    # Dark mode variants
    DARK_MODE_STANDARD = "standard"  # Current implementation
    DARK_MODE_TRUE_BLACK = "true_black"  # Pure black for OLED
    DARK_MODE_DIM = "dim"  # Reduced brightness for night work
    
    _current_dark_mode_variant = DARK_MODE_STANDARD
    _theme_transition_duration = 300  # milliseconds for smooth transitions

    _UI_COLORS_DEFAULT = {
        "accent": "#6B9BD1",        # 강조색 - Modern blue
        "accent_hover": "#7DAEE0",  # 강조색 호버 상태(밝음)
        "accent_pressed": "#5A88BD",# 강조색 눌림 상태(어두움)
        "text": "#E8E8E8",          # 일반 텍스트 색상 - Brighter
        "text_secondary": "#AAAAAA", # Secondary text (68% opacity)
        "text_tertiary": "#777777",  # Tertiary text (47% opacity)
        "text_disabled": "#555555", # 비활성화된 텍스트 색상 (33% opacity)
        "bg_primary": "#1E1E1E",    # 기본 배경색 - Darker for better contrast
        "bg_secondary": "#2D2D2D",  # 버튼 등 배경색 - Slightly lighter
        "bg_hover": "#3A3A3A",      # 호버 시 배경색
        "bg_pressed": "#1A1A1A",    # 눌림 시 배경색
        "bg_disabled": "#252525",   # 비활성화 배경색
        "border": "#404040",        # 테두리 색상 - Softer
    }
    
    # True Black variant for OLED screens
    _UI_COLORS_TRUE_BLACK = {
        "accent": "#6B9BD1",
        "accent_hover": "#7DAEE0",
        "accent_pressed": "#5A88BD",
        "text": "#E8E8E8",
    ...
    # Dim variant for reduced eye strain
    _UI_COLORS_DIM = {
        "accent": "#5A88BD",        # Dimmer accent
        "accent_hover": "#6B9BD1",
        "accent_pressed": "#4A78AD",
        "text": "#B8B8B8",          # Dimmer text
        "text_secondary": "#8A8A8A",
        "text_tertiary": "#676767",
        "text_disabled": "#454545",
        "bg_primary": "#0D0D0D",    # Very dark
        "bg_secondary": "#1A1A1A",
        "bg_hover": "#252525",
        "bg_pressed": "#0A0A0A",
        "bg_disabled": "#151515",
    ...
    def generate_main_button_style(cls):
        """현재 테마에 맞는 기본 버튼 스타일시트를 생성합니다 (Professional shadows and border radius)."""
        return f"""
            QPushButton {{
            ...
            QPushButton:hover {{
                ...
                border-color: {cls.get_color('accent_hover')};
            }}
            QPushButton:pressed {{
                background-color: {cls.get_color('accent_pressed')};
                border-color: {cls.get_color('accent_pressed')};
            }}
            QPushButton:disabled {{
                background-color: {cls.get_color('bg_disabled')};
                color: {cls.get_color('text_disabled')};
                border-color: {cls.get_color('bg_disabled')};
                opacity: 0.5;
            }}
        """
    ...
    def generate_dynamic_height_button_style(cls):
        """수직 패딩이 없고 수평 패딩만 있는 버튼 스타일을 생성합니다 (Professional design)."""
    ...
    def generate_action_button_style(cls):
        """현재 테마에 맞는 액션 버튼(X, ✓) 스타일시트를 생성합니다 (Professional design)."""
        return f"""
            QPushButton {{
                background-color: {cls.get_color('bg_secondary')};
                color: {cls.get_color('text')};
    ...
    def get_color(cls, color_key):
        """현재 테마와 다크 모드 변형에서 색상 코드 가져오기"""
        # Apply dark mode variant if using default theme
        if cls._current_theme == "default":
            if cls._current_dark_mode_variant == cls.DARK_MODE_TRUE_BLACK:
                return cls._UI_COLORS_TRUE_BLACK.get(color_key, cls._UI_COLORS_DEFAULT.get(color_key))
            elif cls._current_dark_mode_variant == cls.DARK_MODE_DIM:
                return cls._UI_COLORS_DIM.get(color_key, cls._UI_COLORS_DEFAULT.get(color_key))
    ...
    @classmethod
    def set_dark_mode_variant(cls, variant):
        """Set dark mode variant (standard, true_black, dim)"""
        valid_variants = [cls.DARK_MODE_STANDARD, cls.DARK_MODE_TRUE_BLACK, cls.DARK_MODE_DIM]
        if variant in valid_variants:
            cls._current_dark_mode_variant = variant
            # Trigger theme callbacks to update UI
            for callback in cls._theme_change_callbacks:
                callback()
            logging.info(f"Dark mode variant changed to: {variant}")
            return True
        return False
    
    @classmethod
    def get_dark_mode_variant(cls):
        """Get current dark mode variant"""
        return cls._current_dark_mode_variant
    
    @classmethod
    def get_available_dark_mode_variants(cls):
        """Get list of available dark mode variants"""
        return [
            (cls.DARK_MODE_STANDARD, "Standard Dark"),
            (cls.DARK_MODE_TRUE_BLACK, "True Black (OLED)"),
            (cls.DARK_MODE_DIM, "Dim Mode")
        ]
    
    @classmethod
    def generate_tooltip_style(cls):
        ...
        return f"""
            QToolTip {{
                ...
                color: {cls.get_color('text')};
                border: 1px solid {cls.get_color('border')};
                border-radius: {BorderRadiusSystem.DEFAULT}px;
                padding: {SpacingSystem.TIGHT}px {SpacingSystem.DEFAULT}px;
                font-size: {TypographySystem.SIZE_BODY}pt;
            }}
        """
    
    @classmethod
    def generate_keyboard_number_button_style(cls):
        ...
        return f"""
            QPushButton {{
                ...
                padding: {SpacingSystem.DEFAULT}px;
                min-width: 40px;
                min-height: 40px;
            }}
            QPushButton:hover {{
                ...
                border-color: {cls.get_color('accent')};
            }}
            QPushButton:pressed {{
                background: #252525;
                border-color: {cls.get_color('accent')};
    ...
    @classmethod
    def generate_badge_counter_style(cls):
        """Badge-style counter for file count"""
        return f"""
            QLabel {{
                background-color: {cls.get_color('bg_secondary')};
                border: 1px solid {cls.get_color('border')};
                border-radius: 12px;
                color: {cls.get_color('accent')};
    ...
    @classmethod
    def generate_gradient_divider_style(cls):
        """Gradient horizontal divider line"""
        return f"""
            QFrame[frameShape="4"] {{
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 transparent,
                    stop:0.5 {cls.get_color('border')},
                    stop:1 transparent);
                max-height: 1px;
                border: none;
            }}
...
class FilenameLabel(QLabel):
    ...
    def setText(self, text: str): # 이 메서드는 PxgateApp에서 직접 호출 시 주의
...
class ZoomScrollArea(QScrollArea):
    def __init__(self, parent=None):
        super().__init__(parent)
        # 부모 참조 저장 (PxgateApp 인스턴스)
        self.app_parent = parent

    def wheelEvent(self, event: QWheelEvent):
        # 부모 위젯 (PxgateApp) 상태 및 마우스 휠 설정 확인
...
class GridCellWidget(QWidget):
    ...
    def get_parent_app(self):
        """부모 위젯을 타고 올라가면서 PxgateApp 인스턴스 찾기"""
        try:
...
class ImageLoader(QObject):
    ...
    decodingFailedForFile = Signal(str) # 디코딩 실패 시 PxgateApp에 알리기 위한 새 시그널(실패한 파일 경로 전달)
    ...
    def __init__(self, parent=None, raw_extensions=None):
        ...
        self._raw_load_strategy = "preview" # PxgateApp에서 명시적으로 설정하기 전까지의 기본값
...
class ThumbnailDelegate(QStyledItemDelegate):
    """썸네일 아이템의 렌더링을 담당하는 델리게이트 (Professional skeleton loading)"""
    ...
    def __init__(self, parent=None):
        ...
        self._skeleton_pixmap = self._create_skeleton_placeholder()
        self._animation_offset = 0  # For skeleton shimmer effect
        
        # Animation timer for skeleton shimmer
        self._animation_timer = QTimer()
        self._animation_timer.timeout.connect(self._update_animation)
        self._animation_timer.setInterval(50)  # 20 FPS for smooth animation
    
    def _create_placeholder(self):
        """플레이스홀더 이미지 생성 (Simple fallback)"""
        size = UIScaleManager.get("thumbnail_image_size")
        pixmap = QPixmap(size, size)
    ...
    def _create_skeleton_placeholder(self):
        """프로페셔널 스켈레톤 플레이스홀더 생성 (Animated shimmer)"""
        size = UIScaleManager.get("thumbnail_image_size")
        pixmap = QPixmap(size, size)
        pixmap.fill(QColor(ThemeManager.get_color('bg_secondary')))
        ...
        gradient.setColorAt(0.0, QColor(ThemeManager.get_color('bg_secondary')))
        gradient.setColorAt(0.5, QColor(ThemeManager.get_color('bg_hover')))
        gradient.setColorAt(1.0, QColor(ThemeManager.get_color('bg_secondary')))
        painter.fillRect(0, 0, size, size, gradient)
        
        # Draw centered icon placeholder
        icon_size = size // 3
        icon_rect = QRect((size - icon_size) // 2, (size - icon_size) // 2, icon_size, icon_size)
        painter.setPen(QPen(QColor(ThemeManager.get_color('text_tertiary')), 2))
        painter.setBrush(Qt.NoBrush)
        painter.drawRoundedRect(icon_rect, BorderRadiusSystem.DEFAULT, BorderRadiusSystem.DEFAULT)
        
        # Draw simple image icon
        margin = icon_size // 4
        painter.drawLine(icon_rect.left() + margin, icon_rect.bottom() - margin,
                        icon_rect.right() - margin, icon_rect.top() + margin)
        
        painter.end()
        return pixmap
    
    def _update_animation(self):
        """Update animation offset for shimmer effect"""
    ...
    def paint(self, painter, option, index):
        ...
        # --- 3. 이미지 그리기 (Professional loading states) ---
        ...
        if image_path:
            ...
            if pixmap and not pixmap.isNull():
                ...
                # Use skeleton placeholder for loading state
                target_pixmap = self._skeleton_pixmap
                use_skeleton = True
            ...
            if use_skeleton:
                shimmer_rect = QRect(x_pos, y_pos, scaled_pixmap.width(), scaled_pixmap.height())
                self._draw_shimmer_effect(painter, shimmer_rect)
    ...
    def _draw_shimmer_effect(self, painter, rect):
        """Draw animated shimmer effect for loading skeleton"""
        from PySide6.QtGui import QLinearGradient
        ...
        gradient = QLinearGradient(rect.left() - 100 + self._animation_offset, rect.top(),
                                  rect.right() - 100 + self._animation_offset, rect.bottom())
        gradient.setColorAt(0.0, QColor(255, 255, 255, 0))
        gradient.setColorAt(0.4, QColor(255, 255, 255, 0))
        gradient.setColorAt(0.5, QColor(255, 255, 255, 15))  # Subtle highlight
...
class ThumbnailPanel(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.parent_app = parent  # PxgateApp 참조
...
class FileListDialog(QDialog):
    ...
    def on_item_double_clicked(self, item):
        ...
        parent_app = self.parent() # PxgateApp 인스턴스 가져오기

        # 부모가 PxgateApp 인스턴스이고 필요한 속성/메서드가 있는지 확인
        if parent_app and hasattr(parent_app, 'image_files') and hasattr(parent_app, 'set_current_image_from_dialog'):
            try:
                # PxgateApp의 image_files 리스트에서 해당 Path 객체의 인덱스 찾기
...
class SessionManagementDialog(QDialog):
    def __init__(self, parent_widget: QWidget, main_app_logic: 'PxgateApp'): # 부모 위젯과 로직 객체를 분리
        super().__init__(parent_widget) # QDialog의 부모 설정
        self.parent_app = main_app_logic # PxgateApp의 메서드 호출을 위해 저장
    ...
    def populate_session_list(self):
        """PxgateApp의 saved_sessions를 가져와 목록 위젯을 채웁니다."""
    ...
    def update_all_button_states(self): # 새로운 메서드 또는 기존 update_button_states 확장
        ...
        # PxgateApp의 image_files 목록이 비어있지 않을 때만 활성화
...
class PxgateApp(QMainWindow):
    STATE_FILE = "pxgate_data.json" # 상태 저장 파일 이름 정의
    ...
    def __init__(self):
        ...
        # 앱 제목 설정 (Professional UI Enhanced)
        self.setWindowTitle("Pxgate - Professional UI")
        ...
        self.line_before_client_selection = HorizontalLine()
        self.control_layout.addWidget(self.line_before_client_selection)
        self.control_layout.addSpacing(UIScaleManager.get("section_spacing", 20))
        
        # --- Client Selection 섹션 ---
        self.client_selection_button = QPushButton("📋 " + LanguageManager.translate("클라이언트 선택"))
        self.client_selection_button.setStyleSheet(ThemeManager.generate_main_button_style())
    ...
    def handle_raw_decoding_failure(self, failed_file_path: str):
        ...
        if current_path_to_display == failed_file_path:
            ...
            #  ImageLoader가 실패 시 빈 QPixmap을 반환하므로, PxgateApp에서 다시 로드 요청해야 함)
    ...
    def show_first_run_settings_popup_delayed(self):
        ...
        if not accepted_first_run:
            logging.info("PxgateApp: 첫 실행 설정이 완료되지 않아 앱을 종료합니다.")
            
            # 🎯 추가 검증: vibeculling_data.json 파일이 생성되지 않았는지 확인
            state_file_path = self.get_script_dir() / self.STATE_FILE
            if state_file_path.exists():
                logging.warning("PxgateApp: 첫 실행 설정 취소했으나 상태 파일이 존재함. 삭제합니다.")
                try:
                    state_file_path.unlink()
                    logging.info("PxgateApp: 상태 파일 삭제 완료.")
                except Exception as e:
                    logging.error(f"PxgateApp: 상태 파일 삭제 실패: {e}")
        ...
        logging.info("PxgateApp: 첫 실행 설정 완료")
    ...
    def _build_donation_section(self):
        ...
        donation_section.setVisible(False)  # Hide the entire donation section
    ...
    def show_client_selection_dialog(self):
        """클라이언트가 선택한 파일을 복사하는 다이얼로그를 표시합니다."""
        if not self.image_files:
            self.show_themed_message_box(
                QMessageBox.Warning,
                LanguageManager.translate("경고"),
                LanguageManager.translate("이미지를 먼저 불러와주세요.")
            )
            return
        
        dialog = QDialog(self)
        dialog.setWindowTitle(LanguageManager.translate("클라이언트가 선택한 파일 복사"))
        ...
        # 설명 레이블
        instruction_label = QLabel(LanguageManager.translate("클라이언트가 보낸 파일 번호나 이름을 붙여넣으세요:"))
        instruction_label.setStyleSheet(f"color: {ThemeManager.get_color('text')}; font-weight: bold;")
        layout.addWidget(instruction_label)
        
        # 힌트 레이블
        hint_label = QLabel(LanguageManager.translate("예: IMG_1234, DSC_5678, 또는 각 줄에 하나씩"))
        hint_label.setStyleSheet(f"color: {ThemeManager.get_color('text_disabled')}; font-size: 10px;")
        layout.addWidget(hint_label)
        
        # 텍스트 입력 영역
        text_edit = QTextEdit()
        text_edit.setPlaceholderText("IMG_1234\nDSC_5678\nIMG_1240")
        ...
        folder_label = QLabel(LanguageManager.translate("대상 폴더 선택"))
        folder_label.setStyleSheet(f"color: {ThemeManager.get_color('text')}; font-weight: bold;")
        folder_layout.addWidget(folder_label)
        
        # 폴더 경로 표시 및 선택 버튼
        folder_row = QHBoxLayout()
        folder_path_label = QLabel(LanguageManager.translate("대상 폴더:") + " " + LanguageManager.translate("폴더를 선택하세요"))
        folder_path_label.setStyleSheet(f"""
            QLabel {{
                background-color: {ThemeManager.get_color('bg_secondary')};
                color: {ThemeManager.get_color('text_disabled')};
                border: 1px solid {ThemeManager.get_color('border')};
        ...
        browse_button = QPushButton(LanguageManager.translate("폴더 선택..."))
        browse_button.setStyleSheet(ThemeManager.generate_main_button_style())
        browse_button.setFixedWidth(120)
        
        selected_folder = [None]  # Use list to store folder path (mutable)
        
        def select_folder():
            folder = QFileDialog.getExistingDirectory(
                dialog,
                LanguageManager.translate("대상 폴더 선택"),
                str(Path.home())
            )
            if folder:
                selected_folder[0] = folder
                folder_path_label.setText(LanguageManager.translate("대상 폴더:") + " " + folder)
                folder_path_label.setStyleSheet(f"""
                    QLabel {{
                        background-color: {ThemeManager.get_color('bg_secondary')};
                        color: {ThemeManager.get_color('text')};
        ...
        folder_row.addWidget(folder_path_label, 1)
        ...
        cancel_button = QPushButton(LanguageManager.translate("닫기"))
        cancel_button.setStyleSheet(ThemeManager.generate_main_button_style())
        cancel_button.setMinimumWidth(100)
        cancel_button.clicked.connect(dialog.reject)
        
        copy_button = QPushButton(LanguageManager.translate("파일 복사"))
        copy_button.setStyleSheet(f"""
            ...
            QPushButton:hover {{
                background-color: {ThemeManager.get_color('accent_hover')};
                border-color: {ThemeManager.get_color('accent_hover')};
            }}
            QPushButton:pressed {{
                background-color: {ThemeManager.get_color('accent_pressed')};
                border-color: {ThemeManager.get_color('accent_pressed')};
            }}
        """)
        
        def copy_files():
            # Get text and parse file names
            text = text_edit.toPlainText().strip()
            if not text:
                self.show_themed_message_box(
                    QMessageBox.Warning,
                    LanguageManager.translate("경고"),
                    LanguageManager.translate("파일 목록이 비어있습니다.")
                )
                return
            ...
            if not selected_folder[0]:
                self.show_themed_message_box(
                    QMessageBox.Warning,
                    LanguageManager.translate("경고"),
                    LanguageManager.translate("대상 폴더를 선택해주세요.")
                )
                return
            
            # Parse file names (split by comma, newline, or space)
            import re
            file_names = re.split(r'[,\n\s]+', text)
            ...
            for name in file_names:
                # Try to find file by name (with or without extension)
                name_lower = name.lower()
                matched = False
                
                for img_path in self.image_files:
                    img_name = img_path.stem.lower()  # filename without extension
                    img_full = img_path.name.lower()  # filename with extension
                    
                    if name_lower == img_name or name_lower == img_full or name_lower in img_name:
            ...
            if not found_files:
                self.show_themed_message_box(
                    QMessageBox.Warning,
                    LanguageManager.translate("복사 실패"),
                    LanguageManager.translate("파일을 찾을 수 없습니다.")
                )
                return
            
            # Copy files
            dest_folder = Path(selected_folder[0])
            success_count = 0
            failed_count = 0
            
            # Progress dialog
            progress = QProgressDialog(
                LanguageManager.translate("파일 복사 중..."),
                LanguageManager.translate("닫기"),
                0,
                len(found_files),
                dialog
            )
            ...
            # Show result
            result_msg = LanguageManager.translate("{success}개 파일 복사 완료\n{failed}개 파일 찾을 수 없음").format(
                success=success_count,
                failed=len(not_found)
            )
            
            if not_found:
                result_msg += "\n\n" + LanguageManager.translate("찾을 수 없는 파일:") + "\n" + "\n".join(not_found[:10])
                if len(not_found) > 10:
                    result_msg += f"\n... (+{len(not_found) - 10} more)"
            
            self.show_themed_message_box(
                QMessageBox.Information,
                LanguageManager.translate("복사 완료"),
                result_msg
            )
            
            dialog.accept()
        
        copy_button.clicked.connect(copy_files)
    ...
    def generate_combobox_style(self):
        ...
        return f"""
            ...
            QComboBox QAbstractItemView {{
                background-color: {ThemeManager.get_color('bg_secondary')};
                color: {ThemeManager.get_color('text')};
                selection-background-color: {ThemeManager.get_color('accent')};
                selection-color: white;
                border: 1px solid {ThemeManager.get_color('border')};
                border-radius: 6px;
                padding: 4px;
    ...
    def _show_raw_processing_choice_dialog(self, is_compatible, model_name, orig_res, prev_res):
        ...
        # 체크박스 스타일은 PxgateApp의 것을 재사용하거나 여기서 정의
        checkbox_style = f"""
    ...
    def move_current_image_to_folder(self, folder_index, index_to_move=None, context_mode="Off"):
        ...
        try:
            ...
            # PxgateApp의 image_files 리스트도 동기화해야 합니다.
    ...
    def setup_zoom_ui(self):
        """줌 UI 설정 (Professional typography)"""
        # 확대/축소 섹션 제목 - Professional styling
        ...
        if hasattr(self, 'selected_grid_indices') and self.selected_grid_indices:
            ...
            if selected_count > 1:
                ...
                if self.primary_selected_index != -1 and 0 <= self.primary_selected_index < total_images:
                    ...
                    title = f"Pxgate - {selected_filename}"
                else:
                    title = "Pxgate"
        else:
            # 기존 단일 선택 방식 (호환성)
    ...
    def _reset_workspace(self):
        ...
        self.setWindowTitle("Pxgate") # 제목표시줄 초기화
    ...
    def display_current_image(self):
        ...
        try:
            ...
            self.setWindowTitle(f"Pxgate - {image_path.name}")
    ...
    def load_state(self):
        """제이슨 파일에서 애플리케이션 상태 불러오기"""
        logging.info(f"PxgateApp.load_state: 상태 불러오기 시작")
        ...
        if is_first_run:
            logging.info("PxgateApp.load_state: 첫 실행 감지. 초기 설정으로 시작합니다.")
        ...
        try:
            ...
            logging.info(f"PxgateApp.load_state: 상태 파일 로드 완료 ({load_path})")
            ...
            logging.error(f"PxgateApp.load_state: 상태 불러오는 중 예외 발생: {e}", exc_info=True)
            self.show_themed_message_box(QMessageBox.Critical, 
    ...
    def initialize_to_default_state(self):
        ...
        logging.info("PxgateApp.initialize_to_default_state: 앱 상태를 기본값으로 초기화합니다.")
    ...
    def undo_batch_move(self, batch_entries):
        ...
        try:
            ...
            for move_info in reversed(batch_entries):
                ...
                # 2. PxgateApp의 image_files 리스트 직접 복원 (핵심)
    ...
    def redo_batch_move(self, batch_entries):
        ...
        try:
            ...
            for move_info in batch_entries:
                ...
                # 2. PxgateApp의 image_files 리스트에서 직접 제거 (핵심)
                try:
    ...
    def update_ui_texts(self):
        ...
        if hasattr(self, 'client_selection_button'):
            self.client_selection_button.setText("📋 " + LanguageManager.translate("클라이언트 선택"))
...
def get_app_data_dir():
    ...
    - Windows: C:\\Users\\<Username>\\AppData\\Roaming\\Pxgate
    - macOS:   ~/Library/Application Support/Pxgate
    - Linux:   ~/.config/Pxgate
    """
    app_name = "Pxgate"
    home = Path.home()
...
def main():
    ...
    print(f"Pxgate 실행 환경: {'개발' if is_dev_mode else '배포'}, 로그 레벨: {logging.getLevelName(log_level)}")
    ...
    translations = {
        ...
        # Client Selection 관련 번역 키
        "클라이언트 선택": "Client Selection",
        "클라이언트가 선택한 파일 복사": "Copy Client Selected Files",
        "클라이언트가 보낸 파일 번호나 이름을 붙여넣으세요:": "Paste file numbers or names sent by client:",
        "예: IMG_1234, DSC_5678, 또는 각 줄에 하나씩": "Example: IMG_1234, DSC_5678, or one per line",
        "대상 폴더 선택": "Select Destination Folder",
        "대상 폴더:": "Destination:",
        "폴더 선택...": "Select Folder...",
        "파일 복사": "Copy Files",
        "파일 목록이 비어있습니다.": "File list is empty.",
        "대상 폴더를 선택해주세요.": "Please select a destination folder.",
        "파일 복사 중...": "Copying files...",
        "복사 완료": "Copy Complete",
        "{success}개 파일 복사 완료\n{failed}개 파일 찾을 수 없음": "{success} files copied\n{failed} files not found",
        "찾을 수 없는 파일:": "Files not found:",
        "복사 실패": "Copy Failed",
        "파일을 찾을 수 없습니다.": "No files found.",
        "이미지를 먼저 불러와주세요.": "Please load images first.",
